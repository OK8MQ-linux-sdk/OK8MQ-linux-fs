...
function usage()
{
    ...
    echo "eg: ./wifi.sh -i wlan0 -s bjforlinx -p 12345678 "
    echo "eg: ./wifi.sh -i wlan0 -s bjforlinx -p NONE "
    echo " -i : wlan0 or wlan1"
    echo " -s : wifi ssid"
...
ifconfig -a|grep wlan1 |grep -v grep  > /dev/null
if [ $? -eq 0 ]
then
